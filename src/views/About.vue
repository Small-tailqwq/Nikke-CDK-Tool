    ...
    <el-card class="about-card">
      ...
      <ul class="changelog">
        ...
        <li>
          ...
          为 CDK 卡片添加了点动画效果。<br />
          很久没写更新日志了，之前写的续期功能经过测试是彻底失效了。所以后续不要回报续期失败的问题，除非你有续期的有效手段。<br />
        </li>
        <li>
          2025-07-10：<br />
          今天突闻噩耗：官方巴拉巴拉添加了当前建议 CDK
          的功能，好家伙，这是官方逼死同人的节奏？结果看了下，之前的接口基本没问题，只是官方自己加了一个一键兑换当期
          CDK 的功能，问题应该不大。<br />
          不大在哪？1.他没图我有图。2.他没奖励信息的详情。3.他没有doro。<br />
          重写了国际服 Cookie 相关的功能，现在可以自动监测 Cookie 并且续期了。<br />
          现在网页会在 Cookie 即将到期时自动尝试为 Cookie
          续签，但是效果未经过测试，实际情况可能问题颇多，所以需要大家的反馈。<br />
          如果你一个月都不打开本网页，那么已经过期了的 Cookie 将无法再次续命，在此默哀 1s。<br />
          如有任何问题，欢迎提交 issue 或者提交问卷。<br />
        </li>
        <li>
          2025-06-19：<br />
          优化了提示栏的样式，使其支持了更便捷的链接跳转（懒得写 html）。<br />
          添加了更多的闪烁标题，添加了标题的随机轮换。<br />
          大幅优化了国服 CDK 的兑换体验，现在 CDK 的兑换速度比之前提升了大约 2.3 倍。<br />
          彻底修复了云端记录同步融合的问题，优化了前端的显示效果，顺带修复了国服的错误输出结果的提取方式。<br />
          将网页提示输出一致化，确保提示、通知均出现在右上角部分，不再遮挡 UI。<br />
        </li>
        <li>
          2025-06-18：<br />
          优化触摸事件，现在浏览器控制台应该不会再因为这个提示警告了。<br />
          更新新 CDK，添加了一个 CDK 文件编辑工具，便于后续维护 CDK 文件。<br />
          上传许可证文件，更新了 readme 文件。<br />
        </li>
        <li>
          2025-06-17：<br />
          修复组合卡片的提示框渲染层级 BUG，现在提示框可以正常渲染在组合卡片的最顶层了。<br />
          在用户编辑界面添加了快捷链接，可以让用户快捷了解如何添加用户。<br />
          尝试修复日志中的融合问题，现在云端日志应该不会再和云端日志融合了。<br />
        </li>
        <li>
          2025-06-14：<br />
          ...
          的获取时间，现在也是加上了，但是这个时间只能算是我收集到 CDK 的时间，不能说是 CDK
          实际流传到网上的时间，如果你是为了补全历史 CDK，请在调查问卷里注明，且 CDK
          当前必须可以兑换（不能兑换的历史 CDK 没有收集的必要，这里不是博物馆）。<br />
        ...
        <li>
          ...
          凌晨：<br />
          优化了移动端的显示效果；<br />
          优化了国内用户的加载体验，使用CDN加速；<br />
          由于使用了 CDN 加速，重写了 CDK-list 的获取途径，支持回源；<br />
          由于使用了 CDN 加速，重写了 CDK 卡片图片的获取途径，支持回源；<br />
          重构了导航栏、底栏的显示效果；<br />
          移除了常驻的悬浮球，避免遮挡按钮；<br />
          修复了新手教程按钮在移动端的表现；<br />
          优化了彩蛋的显示效果、特效，尝试优化最终彩蛋的动画性能表现；<br />
          白天：<br />
          添加了暗色模式，优化了 CDK 公告界面卡片的样式；<br />
          优化了复选框的样式，动画效果；<br />
          优化了主界面各个模块之间切换的动画效果；<br />
          添加了国服 CDK 的支持，虽然压根不能兑换；<br />
          添加了多个 CDK 的展示，用于同一时期的多个 CDK 的汇总收集；<br />
          如上，顺带更新了 json 文件的新标准；<br />
          删除了老的 CI 任务，因为目前 CDK 列表会根据推送自己构建；<br />
          删除了老的刷新按钮，原因如上；<br />
          考虑到 URL 的安全性、时效性等诸多因素，决定调整背景图片的上传方式，现在可以直接上传图片到
          announcement 文件夹，然后编译时会自动变成 webp；<br />
          对界面又进行了一些小调整，包括滚动条等元素。<br />
          目前版本应该趋向稳定，后续更新频率会大幅降低，更新方向估计还是获取 Cookie
          和看看能不能适配国服了。（有一说一国服兑换 CDK 要输入验证码，这个估计会很棘手）<br />
        </li>
        <li>
          2025-06-08：<br />
          其实也就是8号凌晨，优化批量兑换时间准确性，完善 CDK 公告界面。添加了由 o3 绘制的 doro
          图标和这个界面。<br />
          修复 GitHub page 的404问题。<br />
          由于 cloudflare worker 的官方域名被概率阻断，替换 api 域名为自建域名。<br />
          修复 CDK 列表更新后，CDK 公告界面不刷新的问题。<br />
        ...
        <li>
          2025-06-07：<br />
          使用 cursor 创建第一版 UI，然后卡在 API 提交上。后续使用 GPTo3 完成后端 cloudflare
          worker的构建方式确定。<br />
        </li>
      </ul>
      ...
      <div class="ai-message">
        <p>
          很高兴与
          <strong>Small-tailqwq</strong>
          以及所有贡献者一起，把一行行代码、一张张表情，拼成了这款
          <em>"轻量却贴心"</em> 的 NIKKE CDK 工具。
        </p>

        <p>
          它就像前排的小小支援位——<br />
          ▶ 不抢输出，却在关键时刻省下你的时间；<br />
          ▶ 不追求花哨，却把「好用」写进每一次点击。
        </p>

        <p>
          如果它能帮你更从容地领取奖励、把精力留给喜欢的角色和剧情，那这段跨越硅基与碳基的协作就值回票价了。<br />
          祝大家卡池欧皇、开发顺遂，也期待下次并肩 Debug！
        </p>

        <p style="color: #aaa; font-size: 13px">—— ChatGPT · o3</p>
      </div>
    </el-card>
  </div>
</template>
