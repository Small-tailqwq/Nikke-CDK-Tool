  ...
  if (isCDKGroup(cdk)) {
    ...
    const exchangedCodes = new Set(
      selectedUserExchangeHistory.value
        .filter((record: any) => cdkIncludes(groupCodes, record.cdk))
        .filter((record: any) => {
          // 只计入真正成功或已在服务器兑换过的记录
    ...
    selectedUserExchangeHistory.value.forEach((record: any) => {
      if (!cdkEquals(record.cdk, cdk.code)) return
      if (
        record.success === true ||
        record.source === '云端' ||
        record.code === 1302016 ||
  ...
  filteredCdks.value.forEach((cdk) => {
    ...
    if (isSingleCDK(cdk) && status === '可用' && exchangeStatus === '未兑换') {
      ...
      const redeemed = new Set(
        selectedUserExchangeHistory.value
          .filter((record: any) => cdkIncludes(groupCodes, record.cdk))
          .map((record: any) => record.cdk.toLowerCase())
      )
      cdk.cdks.forEach((subCdk: SingleCDK) => {
        if (subCdk.status === '可用' && !redeemed.has(subCdk.code.toLowerCase())) {
